...
export const addProductCartIncrementAction = async newCartProduct => {
  ...
  const updateProducts = await quantityStockIncrementByProduct(
    products,
    newCartProduct
  );
  store.dispatch({
  ...
  if (
    !carts?.products.find(
      cart =>
        cart?.id?.toString()?.trim() === newCartProduct?.id?.toString()?.trim()
    )
  ) {
    ...
    const updateCart = await existsReplaceWithIncrementProduct(
      carts.products,
      newAddedProduct
    );
...
export const addProductCartDecrementAction = async newCartProduct => {
  ...
  const carts = store.getState()?.carts;
  const products = store.getState()?.products.products;
  const updateProducts = await quantityDecrementByProduct(
    products,
    newCartProduct
  );
  store.dispatch({
  ...
  const updateCart = await existsReplaceWithDecrementProduct(
    carts.products,
    newCartProduct
  );
  const totalQty = await cartTotalItemSum(carts.products);
